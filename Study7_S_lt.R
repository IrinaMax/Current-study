...
datDTl <- data.table(l1,  key= c("wafer", "x", "y", "lot"))
datDTl %>%dim

l1_coef<-datDTl[, list(coef(lm(FBC~cycling))), by = key(datDTl)]  #give a matrix  of 304 with intercept and slop
l1_coef<- rename(l1_coef, c("V1" = "coeff" ))
l1_coef %>% head
write.csv(l1_coef, "dies_coef_l.csv")


# we need to take only every second line to pull the slop, the first lines are intercept
l1_coef_slop <- l1_coef[-seq(1, NROW(l1_coef), by = 2)]
l1_coef_slop %>% dim
l1_coef_slop <-as.data.frame(l1_coef_slop)
l1_coef_slop %>% head
# wafer  x  y       lot        coeff
# 1     2  6 17 CP0938712 0.0019713415
# 2     2  8 16 CP0938712 0.0013381949
# 3     2  9 20 CP0938712 0.0009502713
# 4     2 10 20 CP0938712 0.0023511545
# 5     2 11 16 CP0938712 0.0015772439
# 6     2 12 12 CP0938712 0.0012425304

write.csv(l1_coef_slop, "S_ltpd_dq.csv", row.names = FALSE)
...
df_ls<- read.csv("S_ds_ltpd_full.csv", header = TRUE, sep = ",", na.strings = c("NULL", "NaN", "NA"), stringsAsFactors = TRUE)
df <- df_ls # save original  df as df1
...
nmr<- names(df)
nmr
write.csv(nmr, "S_ltpd_slop_names_fullset.csv")
str(df)
tail(df)
...
write.csv(df, "S_LTPD_DS_clean.csv", row.names=FALSE, na="")
...
write.csv(res, "S_Lasso_LTPD_mod1.csv")
...
cv.lasso
# Results
plot(cv.lasso, main = "S_LTPD_LASSO coefficients capture with type.measure='mae', Lambda min  0.00440637 ")
plot(cv.lasso$glmnet.fit, xvar = "lambda", label = TRUE, main ="S_LTPD_LASSO coefficients capture with type.measure='mae', Lambda min  0.00440637")
plot(cv.lasso$glmnet.fit, xvar = "dev", label = TRUE, main="S_LTPD_LASSO coefficients capture with type.measure='mae', Lambda min  0.00440637")
cv.lasso$lambda.min
#[1] 0.00440637
cv.lasso$lambda.1se
#[1]
...
l_name_coeff_SL 
write.csv(l_name_coeff_SL, "S_coefLasso_LTPD_slop.csv")
# result in D_result of htpd" in Exel"

f <- as.vector(l_name_coeff_SL[,1])
f
...
# add responce 
df$slop
ss$slop <-df_ls$u.slop

write.csv(ss, "S_LTPD_lasso2_subset.csv")
lmod_ss <- lm(slop~., ss)
lmod_ss
summary(lmod_ss)
...
plot(predLTPD, y,  xlab = "prediction", ylab = "SLOP",col = "dark red", main = "Rate of actual LTPD FBC growth against to predicted, 
     based on the lambda min=  1.410251e-05, CV: 10 Fold")            
...
print(best.iter)
f.predict <- predict(df.boost,test[,-351],best.iter)
f.predict %>% plot

f1.predict <- predict(df.boost, train[,-351], n.trees = 1500, type = "response")
f1.predict
...
predmatrix<-predict(df.boost,test[,-354], n.trees = n.trees)
predmatrix
...
y_pred <- predict(df.boost, data.matrix(test[,-354]))
