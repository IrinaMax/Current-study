# fit parametrs with slop 
## This script mean nothing, its my durty work
##   
...
datDT <- data.table(h1,  key= c("wafer", "x", "y", "lot"))
datDT %>%dim
...
write.csv(h1_coef_slop, "S_htpd_dq.csv", row.names = FALSE)
...
df_hs<- read.csv("S_ds_htpd_full.csv", header = TRUE, sep = ",", na.strings = c("NULL", "NaN", "NA"), stringsAsFactors = TRUE)
df <- df_hs # save original  df as df1
...
df6_HTPD_clean <- df
#[1] 151 354
#after additional cleaning and removing 2 row with NA
...
l_name_coeff_SR 
write.csv(l_name_coeff_SR, "S_coefLasso_HTPD_slop.csv")
...
gbm.imp <-  s[1:30,]
gbm.imp
write.csv(gbm.imp, "S_HTPD_GBMachine_subset.csv")
# var  rel.inf
